...
$(function(){
	...
	if ($('body').hasClass('mapfull')) {
		...
		infowindow.panMapIfOutOfWiew = true;
...
function onSelect(event) {
	var bounds = new OpenLayers.Bounds();
	if (event.feature.cluster!=undefined && event.feature.cluster.length>1) {
		...
		// Show tooltip
		//infowindow.showAt(new OpenLayers.LonLat(event.feature.geometry.x,event.feature.geometry.y),event.feature.data);
		infowindow.updatePosition(new OpenLayers.LonLat(event.feature.geometry.x,event.feature.geometry.y),event.feature.data);
	}
}
